...
class Block:
    ...
    record: Record  # Replace the `data` attribute with the `record` attribute of type `Record`
    creator_id: int  # Unique identifier for the creator of the block
    prev_hash: str = "0"  # Hash value of the previous block in the chain
    timestamp: str = datetime.datetime.utcnow().strftime("%H:%M:%S")  # Time when the block is created
    nonce: int = 0  # Nonce value used for mining

    # Define a method to hash the block
    def hash_block(self):
        sha = hashlib.sha256()

        # Encode and update the SHA-256 hash object with the block attributes
...
class PyChain:
    ...
    difficulty: int = 4  # Difficulty level for mining

    # Define a method for proof-of-work
    def proof_of_work(self, block):
        # Calculate the initial hash of the block
        calculated_hash = block.hash_block()

        # Set the number of leading zeros required for the hash
        ...
        # Print the winning hash and return the block with the correct nonce
        print("Wining Hash", calculated_hash)
        return block

    # Define a method to add a block to the chain
    def add_block(self, candidate_block):
        # Perform proof-of-work on the candidate block
        block = self.proof_of_work(candidate_block)
        # Add the block to the chain
        self.chain += [block]

    # Define a method to validate the chain
    def is_valid(self):
        ...
        for block in self.chain[1:]:
            ...
            # Calculate the hash of the current block for the next iteration
            block_hash = block.hash_block()

        # If all blocks are valid, the chain is valid
...
# Display a markdown heading for the PyChain Ledger section
st.markdown("## The PyChain Ledger")

# Create a DataFrame from the PyChain ledger and display it in Streamlit
pychain_df = pd.DataFrame(pychain.chain).astype(str)
st.write(pychain_df)

# Create a slider in the sidebar to adjust the block difficulty
difficulty = st.sidebar.slider("Block Difficulty", 1, 5, 2)
pychain.difficulty = difficulty

# Display a markdown heading for the Block Inspector section in the sidebar
st.sidebar.write("# Block Inspector")

# Create a select box in the sidebar to choose a block to inspect
selected_block = st.sidebar.selectbox(
    "Which block would you like to see?", pychain.chain
)

# Display the selected block's details in the sidebar
st.sidebar.write(selected_block)

# Create a button to validate the chain
if st.button("Validate Chain"):
    # Check if the chain is valid and display the result in Streamlit
